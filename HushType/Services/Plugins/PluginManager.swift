...
/// Manages discovery, loading, activation, and lifecycle of HushType plugins.
///
...
/// - Conform to `HushTypePlugin` (and optionally `ProcessingPlugin` or `CommandPlugin`)
///
/// Usage:
/// ```swift
...
final class PluginManager {
    ...
    init() {
        ...
        self.pluginsDirectory = appSupport.appendingPathComponent("HushType/Plugins")
    }
    ...
    ///   if the principal class does not conform to `HushTypePlugin`.
    ...
    func loadPlugin(at url: URL) throws {
        ...
        guard let plugin = instance as? any HushTypePlugin else {
            throw PluginError.loadFailed(
                path: url.path,
                reason: "Principal class '\(principalClass)' does not conform to HushTypePlugin"
            )
        }

        // Verify API major version compatibility
        let requiredMajor = kHushTypePluginAPIVersion.split(separator: ".").first.map(String.init) ?? "0"
        ...
        guard requiredMajor == pluginMajor else {
            throw PluginError.incompatibleVersion(
                ...
                required: kHushTypePluginAPIVersion,
